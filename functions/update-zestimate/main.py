...
def lambda_handler(event, context):
    ...
    data = pd.read_csv(
        s3fs.open("{}/{}".format(bucket_name, bucket_key), mode="rb"), index_col=False
    )

    # fetch Zestimate
    url = (
        "https://api.bridgedataoutput.com/api/v2/zestimates_v2/zestimates?access_token="
        + zwsid
        + "&limit=1&near="
        + zaddress
    )
    ...
    if response.status_code != 200:
        logger.fatal(
            "Received unexpected response (%s) from Zillow API" % response.status_code
        )
        return
    ...
    if parse(zestimate_updated, ignoretz=True).date() <= parse(data.iloc[-1, 0]).date():
        logger.info(
            "No update necessary - %s is not newer than %s. Exiting..."
    ...
    response = client.publish(
        ...
        Message="Current zestimate is "
        + "${:,.0f}".format(int(zestimate))
        + " (change of "
        + "${:,.2f}".format(int(zestimate) - int(data.iloc[-1, 2]))
        + "). "
        + "Low-High: {:.1%} - {:.1%}.".format(
            int(zestimate_low) / 100, int(zestimate_high) / 100
        ),
    )
